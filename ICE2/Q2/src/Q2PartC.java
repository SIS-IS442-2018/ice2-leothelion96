...
public class Q2PartC {
    ...
    public static String getFarmerDetails(CommunityGarden garden, int plotNumber) {
        ...
        int totalPlot = garden.getTotalPlots();
        if (plotNumber > totalPlot) {
            return "N.A.";
        }
        else {
            if (garden.getPlot(plotNumber).getFarmer() != null) {
                Plot p1 = garden.getPlot(plotNumber);
                return p1.getFarmer().getName() + "[" + p1.getFarmer().getGender() + "]";
            } else {
                return "available";
            }
        }
